...
| 2026-09-12 | T3 | `IconPixmap` read over the bus against the real four-state tray (T3's code, not the plan's prototype), a live `wisp run --stub --backend plain` in an isolated `XDG_RUNTIME_DIR=/run/user/1000/wisp-t3`, item selected by the `wisp-hud` pid: property type `a(iiay)`, value `[(48, 48, 9216), (32, 32, 4096), (24, 24, 2304), (22, 22, 1936), (16, 16, 1024)]` — the Waiting tile (`--stub` publishes no `log`), five sizes, largest first, matching `n·n·4` exactly. `wisp stop` under the same env exited 0 and left no `wispd`/`wisp-hud` pids. wisp-hud unit test count 87 → **93** (the six new tests: `the_icon_is_waiting_tailing_or_fighting_by_the_log_and_the_fight`, `error_wins_over_every_other_state`, `every_state_carries_five_square_pixmaps_of_the_right_length`, `the_four_states_are_four_different_pictures`, `publish_nudges_when_only_the_icon_changed`, `the_layout_error_reaches_the_tray_state_from_the_render_loop`); every other crate's count unchanged (wisp 46+36+1, wisp-config 68, wisp-probe 15, wisp-proto 23, wispd 106(3 ignored)+9) | Appendix B recipe with `<task>` = `t3`; `busctl --user --json=short get-property "$item" /StatusNotifierItem org.kde.StatusNotifierItem IconPixmap \| python3 -c "…"`; `cargo build --workspace --locked && timeout 300 cargo test --workspace --locked` |
| 2026-09-12 | T2 | `install.sh` into a scratch prefix, from a staged tarball root: install prints nine files under `share/icons` (the eight PNGs, `512x512` down to `16x16`, plus the one `scalable` SVG); uninstall then leaves zero files under the scratch prefix. Repeated against the real tarball produced by `release.sh`: same nine-then-zero result | `packaging/install.sh --prefix <scratch>` then `--uninstall`, `find … -type f \| sort`, once from a hand-staged tarball root and once from the tarball `release.sh` actually built |
| 2026-09-12 | T2 | `release.sh` end to end on `0.3.0-beta.3` (beta channel, `latest-pre`) produced all four `dist/` paths (tarball, AppImage, `.zsync`, `SHA256SUMS`) with `release.sh: channel beta (latest-pre, betas only)` on stderr, exit 0. The tarball (`wisp-0.3.0-beta.3-x86_64-linux.tar.gz`) holds the ten files it always has plus eight PNGs under `icons/hicolor/<n>x<n>/apps/`. `appimagetool --appimage-extract`'s root listing is exactly `AppRun`, `.DirIcon`, `io.github.jds300.Wisp.desktop`, `io.github.jds300.Wisp.png`, `usr` — **no root `.svg`**; `.DirIcon` is a symlink to `io.github.jds300.Wisp.png`; `find usr/share/icons` lists the eight PNGs plus the one scalable SVG (nine files); `--version` prints `wisp 0.3.0-beta.3` | `packaging/release.sh`; `cd dist && APPIMAGE_EXTRACT_AND_RUN=1 ./Wisp-*.AppImage --appimage-extract`, `ls -a squashfs-root`, `find squashfs-root/usr/share/icons -type f`, `ls -l squashfs-root/.DirIcon`, `./Wisp-*.AppImage --version`; `tar -tzf dist/wisp-*.tar.gz` |
| 2026-09-12 | T2 | The offline Flatpak build ran to completion on this box: `packaging/flatpak/build.sh`'s `cargo-sources.json` staleness check passed untouched (no dependency changed), `org.flatpak.Builder` was already installed, and `flatpak run org.flatpak.Builder --user --install --force-clean --install-deps-from=flathub` exited 0, exporting all eight `share/icons/hicolor/<n>x<n>/apps/io.github.jds300.Wisp.png` plus the metainfo. Post-install, `flatpak run --command=find io.github.jds300.Wisp /app/share/icons/hicolor -type f` lists the eight PNGs and the scalable SVG (nine files, none missing); `flatpak run io.github.jds300.Wisp --version` prints `wisp 0.3.0-beta.3` | `packaging/flatpak/build.sh`; `flatpak run --command=find io.github.jds300.Wisp /app/share/icons/hicolor -type f \| sort`; `flatpak run io.github.jds300.Wisp --version` |
| 2026-09-12 | T5 (fix wave) | **F1 — the two green crops were the wrong way round.** Before the fix, mean ARGB over each `tray-<state>-48.argb` was tailing (255, 56.1, 84.9, 67.9) and fighting (255, 46.5, 68.0, 56.4) — the glowing sheet tile ("Active") fed Tailing and the flatter one ("Active (Variant)") fed Fighting, the opposite of the spec's and README's "green"/"bright green" words. `packaging/render-icons.sh`'s crop boxes were swapped (fighting ← `+80+1105`, tailing ← `+770+1105`); after the swap, mean ARGB is tailing (255, 46.5, 68.0, 56.4) and fighting (255, 56.1, 84.9, 67.9) — Fighting is now the brighter tile, matching the prose. Exactly the ten tailing/fighting `.argb` files (5 sizes × 2 states) changed on the re-run; the other nineteen outputs stayed `unchanged`; a further re-run wrote 0 files | `bash packaging/render-icons.sh` before and after editing the crop boxes, `git status --short`; a python loop averaging the four bytes-per-pixel of each `tray-<state>-48.argb` |
| 2026-09-12 | T5 (fix wave) | **F3 — a repo-root `.gitattributes`, outside the plan's File Structure.** Eleven of the twenty `.argb` files were git-detected as text with no `.gitattributes` present, so any eol conversion on clone or checkout would corrupt them and break invariant 1's clean `git status` after a re-run. Added `.gitattributes` with `*.argb binary`, `*.png binary`, `*.jpeg binary`, `*.jpg binary`. After adding it, `git check-attr binary` reports `binary: set` for every one of the twenty `.argb` files (spot-checked `tray-error-24.argb`), the sheet, the banner and a hicolor PNG; a further `packaging/render-icons.sh` run still writes 0 files and `git status --short` shows only the new `.gitattributes` and the already-committed F2 change. Cost if wrong: none — the file only declares what these files already are | `git check-attr binary <each path>`; `bash packaging/render-icons.sh && git status --short` |
...
7. Implementers worked in isolated git worktrees off spec-7-the-face, two at a time, controller fast-forwarding. (controller) — Cost if wrong: one rebase per collision; none occurred.
8. The plan writer's two file-ownership deviations were accepted before dispatch — T3 deletes the old pixmaps, T4 writes the sheet's PROVENANCE entry. (controller) — Cost if wrong: none.
9. Spec 4 §"Release artifacts"'s "ten files" tarball line is superseded, not edited: the root still carries those ten and now an icons/hicolor tree of eight PNGs, eighteen files in total; Spec 7 §4.2 is the current authority on the tarball's contents. (T5) — Cost if wrong: a reader of Spec 4 counts ten and finds eighteen with nothing saying which document is current.
10. docs/STATUS.md holds the three moved sections byte for byte modulo link re-rooting, the only class of change and forced by the directory. (T4, verified by T5) — Cost if wrong: a reader takes "unchanged" literally; the diff shows prose was not altered.
11. The README orders the tarball before the Flatpak, against spec §4.4's ordering; the Flatpak paragraph's gamescope caveat reads best last and no acceptance criterion names an order. (T4, ratified by T5) — Cost if wrong: a Flatpak user reads one paragraph further.
12. The two green crop boxes were swapped in the fix wave so the sheet's glowing tile is Fighting and the flatter tile is Tailing, making the spec's and README's "green / bright green" words true of the shipped tiles. (controller, T5) — Cost if wrong: JDS300 preferred the glow for tailing; a two-line swap at Milestone 5.
13. A `.gitattributes` marking `*.argb`, `*.png`, `*.jpeg` and `*.jpg` binary was added in the fix wave, outside the plan's File Structure, because eleven pixmaps were git-detected as text. (T5) — Cost if wrong: none; it declares what the files already are.
14. Two acceptance rows rest on the implementers' evidence rather than a T5 re-run: the 100×100 sheet refusal (T1 Step 7) and the live IconPixmap read (T3 at 1bd35cc, whose HUD binary is byte-identical to head's since only packaging changed after). (T5) — Cost if wrong: an acceptance row rests on one run; Milestone 5 exercises the bus path on real hardware.
15. **Accepted, Minor 6: `render-icons.sh` writes the banner before the launcher and tray size asserts run.** A re-laid-out 2816×1536 sheet could therefore leave a new `banner.png` on disk before a later assert exits 2. Not a spec violation — the script still refuses on a bad sheet, one file just lands first — and no acceptance criterion orders the writes. (T5) — Cost if wrong: a partial write on a future sheet change, visible in `git status` and overwritten by the next correct run.
16. **Accepted, Minor 7: the tarball's README carries relative links into `docs/`** that the tarball does not ship. Pre-existing since Spec 4 and out of Spec 7's scope. (T5) — Cost if wrong: a tarball user's README link goes nowhere; the GitHub-hosted README, with working links, is one click away.
17. **Accepted, Minor 8: `install.sh --uninstall` can remove a pre-existing *empty* directory it installed into.** Pre-existing Spec 4 semantics, checked adversarially by T2 and out of Spec 7's scope. (T5) — Cost if wrong: an empty directory a user made themselves under the install prefix disappears on uninstall; nothing inside it is touched.
