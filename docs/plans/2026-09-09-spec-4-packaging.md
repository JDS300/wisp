...
| Present on `PATH` | `gamescope`, `mangoapp`, `desktop-file-validate`, `appstreamcli`, `zsyncmake` (`zsync` 0.6.6-1.1), `curl` (8.21.0), `jq`, `strip`, `socat` (`/usr/bin/socat`), `python3` 3.14.7, `uv` |
...
| Test counts today (glibc) | wispd 82 passed / 3 ignored, wisp-hud 31, wisp-proto 11 — **before implementation**; the merged counts are in Appendix A |
| `cargo fmt --all --check` | diffs in **15 of the tree's 18** `.rs` files before implementation (wispd 8, wisp-hud 6, wisp-proto 1), and in **30 of 34** as merged at `f692794` — every crate, including the three new ones. No fmt gate is added by this plan; do not reformat anything, and do not add a gate that fails on arrival |
...
`.gitignore` already carries `/dist/` and `target/` from before this plan — Task 8 does not add a second line for either. Nothing in this plan commits build output, the cached tooling, or `cargo-sources.json`'s generator.
...
Two things about that block will legitimately differ once this task is done, and neither is a failure: the `filtered out` count moves with the tests this task adds **and removes**, and `tests/logs_dir.rs` adds a second `Running` block with its own result line, in which the two ignored replay tests do not appear. **As merged, this step reports `91 filtered out`** — wispd ended Task 5 with 93 tests, 90 runnable and 3 ignored, and the run selects 2. This plan first predicted 90 (83, plus the eight unit tests Task 3 adds in `wispd/src/source.rs` and `timers.rs`, minus the one deleted with `spells_dir_from_log`); Task 3's fix wave added one more test to `wispd` itself, and the merged number is the one to check against. Note that the `wisp_config::source` tests do **not** move this count, because they live in another package. The timing line always differs. If either replay test *fails*, the switch work has changed a parsing rule. Stop, report both the failure and the diff, and do not adjust a number to make it pass — Appendix A and Spec 3 §6 are the arbiters.
...
```
  ...
  **A flag given twice** is handled differently by the two commands, and both land on "the first wins." `doctor_command` accumulates each value flag with `Option::get_or_insert`, so its own first occurrence sticks; the assertion for it lives inside `doctor_takes_the_five_value_flags_and_nothing_else`, not a separate test. `run` decides nothing here — `partition_run_flags` forwards every occurrence to the child verbatim, in the order given — and it is the child's own `iter().position(…)`, in `wispd` and in `wisp-hud`, that keeps the first. A report that disagreed with the launch it describes would be worse than no report, which is why `doctor`'s rule has to match the child's rather than invent its own.
...
```
  ...
  `find_child` skips a candidate that is not executable — in **both** places it looks, beside the current executable and on `PATH` — using `std::os::unix::fs::PermissionsExt`: a regular file with any execute bit. No `libc`, and no spawning something that cannot be spawned. Split the two-places rule into a helper taking `(name, beside, path)` so both are testable without mutating the process environment, which every other test in the binary is using. `wait_for_daemon` polls `child.try_wait()` and a short connect attempt every 100 ms until the timeout, so a daemon that dies at once is reported at once; `wait_for_socket` is the socket-only form, and the two share one loop so neither can drift on the interval or the deadline. Once `wait_for_daemon` exists, `wait_for_socket` is called only by its own tests, so it carries `#[allow(dead_code)]` with the same comment `Server::client_count` uses — `wisp` is a binary crate, and an otherwise-unused public function trips `dead_code` on a plain `cargo build`.
...
```
  ...
  **Doctor stats what it prints, and a log that is not there is a failure.** A `LogSource::File` whose path is not a file prints `log:       <path> (config log, missing)` — or `(--log flag, missing)` when the flag named it — and **exits 1**, because that is exactly what `wispd` does with the same config and doctor must not report an unlaunchable setup as healthy. The stat is `is_file()`, **not** `exists()`: a directory sitting at the log path also reads `missing`, since wispd cannot tail it either, and `exists()` would report a directory as a log the daemon can open. A `logs_dir` whose directory holds no `eqlog_*.txt` prints `newest of 0 files` and also exits 1. Everything else on the list is already a stat: the config file's existence, `spells_us.txt`, whether the socket answers. The spells directory comes from `wisp_config::source::resolve_spells_dir` — the same call `wispd` makes — and never from a restated chain.
...
```bash
git add Cargo.toml Cargo.lock crates/wisp crates/wispd crates/wisp-proto crates/wisp-config
git commit -m "$(cat <<'EOF'
...
2. Require `zsyncmake` and `curl` on `PATH`, each with its own `command -v` guard: `release.sh: zsyncmake not found (package: zsync); appimagetool -u writes no .zsync without it`; `release.sh: curl not found (package: curl)` — `curl` fetches the two pinned tooling files in the next step. Neither is a warning — the spec's acceptance requires four files in `dist/`.
...
8. `APPIMAGE_EXTRACT_AND_RUN=1` exported, then invoke the cached appimagetool with `--runtime-file <cached runtime>`, `-u 'gh-releases-zsync|JDS300|wisp|latest|Wisp-*-x86_64.AppImage.zsync'`, `ARCH=x86_64`, the AppDir and the output path `dist/Wisp-<v>-x86_64.AppImage`. appimagetool 1.9.1 writes the `.zsync` into its current working directory, not beside the output path, so run it from inside `dist/` with a relative output name (`Wisp-<v>-x86_64.AppImage`) and an absolute path to the AppDir.
...
**`packaging/install.sh`** — `set -euo pipefail`. `--prefix <dir>` (default `$HOME/.local`), `--uninstall`, `--help`. Install: the three binaries to `<prefix>/bin/`, the desktop file to `<prefix>/share/applications/`, the SVG to `<prefix>/share/icons/hicolor/scalable/apps/`, the metainfo to `<prefix>/share/metainfo/`; `install -D` semantics, creating directories, overwriting in place so a re-run is idempotent. Uninstall: remove exactly those **six** paths, then, for each of the four install directories in turn, walk upward toward the prefix `rmdir`-ing directories for as long as each one is now empty, stopping the walk the moment a directory is not empty and always stopping before the prefix itself — `rmdir` only, never `rm -r`, and the prefix is never removed. (The earlier "then any of the four directories that are now empty" reading only cleared the leaf directories and measurably left `share/icons/hicolor/scalable` behind.) It runs from inside the extracted tarball and finds its own files relative to `$0`, resolved through symlinks with `readlink -f "$0"` first, so it works from anywhere the tarball was unpacked and also when `install.sh` itself is invoked through a symlink.
...
The first `<p>` opens with the tagline and continues into a full sentence. **No screenshots** — a Flathub requirement deferred with the submission. Measured output: `✔ Validation was successful: pedantic: 1`, exit 0 — the one pedantic item is `cid-contains-uppercase-letter` for `io.github.jds300.Wisp`, inherent to the id the spec fixes, and is accepted rather than "fixed". **Warnings are not acceptable to this tool and are not acceptable here** — a pedantic item is not a warning, and `appstreamcli` exits non-zero on an actual warning — so fix anything the tool reports as a warning rather than recording it as cosmetic.
...
**`packaging/check-container.sh`** — the outer script, `set -euo pipefail`; runs the spec's Clean-container criterion inside `docker run --rm -v "$PWD/dist:/dist:ro" ubuntu:24.04 bash -c '…'`, extracting the tarball to a temp dir inside the container and running `apt-get update && apt-get install -y --no-install-recommends xvfb` before the last check. It prints `PASS <check>`, or `FAIL <check> <reason>`, one per line, on stdout, and exits non-zero if any failed. **The inner container script — the string passed to `bash -c` — deliberately does not `set -e`**: one failed check must not hide the others, so each check is its own `if`/`else` and the script keeps going after a `FAIL` to run the rest.

**Before touching Docker at all, it checks `dist/` exists.** `[ -d dist ]`; if not, print `FAIL sums dist/ is missing; run packaging/release.sh first` and exit 1 without invoking `docker run`. Reason: a bind mount whose host source does not exist makes Docker create it as root, which then breaks a later `packaging/release.sh` with a permission error trying to write into `dist/` — measured on the development box.

**The sums check and the four numbered checks — five `PASS` lines in total — share one container and one daemon, in this order.** First the script verifies the artifacts it was given, in the same subshell form Step 4 uses: `( cd /dist && sha256sum -c SHA256SUMS --quiet )`. Extracting the tarball into the temp dir is guarded explicitly next: on failure, print `FAIL version could not extract the tarball` and exit 1 before check 1 runs — the label is `version` because that check is the first thing that would have needed the extracted binary. Then: (1) `wisp --version` prints `wisp <version>`. (2) Start `wispd --stub &`, record its pid, then assert `wisp status --json` prints one line that decodes as v3 (`grep -q '"v":3'`). (3) With **that same daemon still running**, assert `wisp doctor` exits 1, its output names the config path — `/root/.config/wisp/config` in this container, since nothing there sets `XDG_CONFIG_HOME` and the `wisp/` subdirectory is part of the path, not of the filename — and its output contains the exact string `a daemon is listening` (`crates/wisp/src/doctor.rs`), since this is the only check that runs doctor against a live daemon. Exit 1 is about the absent log, not the socket, and doctor must truthfully report the daemon as listening. (4) Assert the daemon from check 2 is still alive with `kill -0 <pid>`; if it is not, print `FAIL hud-under-xvfb: the stub daemon from check 2 is gone` and do not start the HUD. If it is, `timeout 5 xvfb-run wisp-hud --backend plain` must exit **124** — the HUD never exits on its own, so the timeout is the pass condition. Kill the daemon at the end; the container is `--rm`, so nothing outlives the script.

**On any `FAIL`, the relevant captured log follows it on stderr.** `wispd`'s stderr (redirected to a file when it is started in check 2), the `apt-get install` log, and the HUD's stderr are each captured to a file as the script runs; a `FAIL` line prints that check's captured file to stderr immediately after, because the container is `--rm` and nothing else survives it. Stdout stays `PASS`/`FAIL` lines only, so a caller that greps stdout for `^FAIL` never sees log noise mixed in.
...
- [ ] The tarball holds ten files, `install.sh` is `0755` inside it, and `install.sh --prefix <tmp>` then `--uninstall` leaves a fresh prefix exactly as it was — empty, `<tmp>` itself still there (test it) — and the same round trip works when `install.sh` is invoked through a symlink (`ln -s` it into another directory first, run the link).
- [ ] A pre-existing file survives: seed `<tmp>/share/icons/hicolor/scalable/apps/other.svg` before `install.sh --prefix <tmp>`, then `--uninstall`; confirm `other.svg` and every directory on its path are still there afterward.
...
`regen-cargo-sources.sh` takes an optional output path as its one argument, defaulting to the committed `packaging/flatpak/cargo-sources.json`; `build.sh`'s staleness check calls it with a temp file so its regeneration never touches the committed one. The generator's own download is skipped when the cached copy under the cache directory is already present — the URL is content-addressed by the pinned commit, so a cache hit can never be stale.
...
**`build.sh`** — first checks that the committed sources still describe the lock file: regenerate to a temp file with `regen-cargo-sources.sh` (or the same command it wraps) and `cmp` it against `packaging/flatpak/cargo-sources.json`; on any difference exit 1 telling the user to run `regen-cargo-sources.sh` and commit the result. An offline build against stale sources fails deep inside cargo with a message that does not say what is wrong, so this check is the diagnostic.

Two prerequisites for a fresh box, each guarded and idempotent so a repeat run does nothing. `flatpak remote-add --user --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo`: the development box had `flathub` configured system-scope only, which a `--user` install does not see. `flatpak install --user -y --noninteractive flathub org.flatpak.Builder` when `flatpak info --user org.flatpak.Builder` fails: `flatpak run` does not install the app it runs, and `--install-deps-from=flathub` on the Builder invocation below covers only the *manifest's* dependencies — the Platform, the Sdk, `rust-stable` — never the Builder application itself.

`build_dir="$(mktemp -d "$root/.flatpak-build-XXXXXX")"` under the repository root, never `/tmp`: flatpak-builder needs its state directory and its build directory on one filesystem, and on the development box `/tmp` is tmpfs while the worktree is btrfs — the `/tmp` form was tried first and failed with exactly that cross-filesystem error, measured. `state_dir="$root/.flatpak-builder"` is a **sibling** of `build_dir`, not nested inside it: `flatpak-builder --force-clean` refuses to run with `Refusing to delete current working directory, state directory or their parents` when the state directory sits inside the build directory it is about to wipe — measured directly, from a `--state-dir` nested under `build_dir`. `state_dir` is `rm -rf`'d before the build starts, for a clean state on every run, and `--state-dir="$state_dir"` is passed explicitly so its location never depends on the caller's working directory. An `EXIT` trap removes both `build_dir` and `state_dir`; it fires on an untrapped `SIGINT` (bash runs `EXIT` handlers there too), so `Ctrl-C` cleans up both, but a `SIGKILL` or a crash does not, and a leftover `.flatpak-build-*` directory or `.flatpak-builder` is the evidence when that happens. `.gitignore` gains `/.flatpak-build-*/` and `/.flatpak-builder/` — anchored to the repository root and directory-only — and Task 7's Step 5 `git add` line gains `.gitignore`.

Then `flatpak run org.flatpak.Builder --user --install --force-clean --install-deps-from=flathub --state-dir="$state_dir" "$build_dir" packaging/flatpak/io.github.jds300.Wisp.yml`, using `org.flatpak.Builder` from Flathub because the development box has no native `flatpak-builder`. `--install-deps-from=flathub` is required, not optional: the Platform, the Sdk and `rust-stable` are all absent today, so the first run downloads them. With `--bundle`, also run `flatpak build-bundle` to `dist/Wisp-<v>.flatpak`.
...
The third check is the socket-sharing rule: instance B must print a v3 line, which only works if both resolve `$XDG_RUNTIME_DIR/app/io.github.jds300.Wisp/wispd.sock`. **`check.sh` never runs `flatpak kill io.github.jds300.Wisp`** — that kills every running instance of the app for the user, including one wrapping a live game launch, which is not this script's to touch. It kills only the `flatpak run` wrapper's own pid, recorded from the `&` in the instance-A line, ending that one sandboxed instance and nothing else. Each check's stdout and stderr are captured into a `mktemp -d` log directory, removed by the same `EXIT` trap `build.sh` uses.

The fourth check's expected socket name is derived, not hard-coded: `DISPLAY` with its leading `:` stripped and everything from the first `.` onward dropped — `:0.0` becomes `X0` — matching X's own `/tmp/.X11-unix/X<n>` naming.
...
**README.md** — the status table's row 4 becomes a link to the spec with its real state, in the house wording of rows 1–3: implemented, what was verified automatically, what is pending JDS300, and a link to this plan. Add an **Installing** section after "How it draws over the game", led by two prebuilt artifacts and one local build — not three prebuilt ways, which is false for the Flatpak: the tarball plus `install.sh` (with `--prefix` and `--uninstall`) and the AppImage (including that it needs the kernel's FUSE and that `--appimage-extract-and-run` is the fallback) need no clone; the local Flatpak build (`packaging/flatpak/build.sh`) does, since it compiles the workspace itself from the offline-vendored sources, and its first run downloads **four** things, not three — the Platform, the Sdk, `rust-stable` and `org.flatpak.Builder` itself. Then the two-step first run: `wisp config set logs_dir <the game's Logs directory>` and `wisp run`; with the AppImage, its own path stands in for `wisp` in both — `./Wisp-<version>-x86_64.AppImage config set logs_dir <the game's Logs directory>` and `./Wisp-<version>-x86_64.AppImage run`, `<version>` the placeholder, not a literal number — plus `wisp run -- %command%` as a Steam or Lutris launch option, and `wisp doctor` as the thing to run when it does not work. State the AppImage, not the Flatpak, is the artifact for wrapping a gamescope launch on a desktop.
...
- **`flatpak/flatpak-builder-tools`** — the cargo generator at commit `1fc32195e3e60fe5c97f0af646dec7a99df5962b`, named explicitly here as in `THIRD_PARTY.md`, not left as "a pinned commit"; MIT declared in the file itself (`__license__ = "MIT"`); the repository has no root `LICENSE` at that commit, which is recorded rather than papered over. Tool used, nothing vendored.
...
**THIRD_PARTY.md** — a new "Packaging tooling" section recording, as *used, not vendored*: `appimagetool` 1.9.1 (MIT, fetched at a pinned SHA-256 into a cache outside the repository); `AppImage/type2-runtime` release `20251108` (MIT, **embedded in the AppImages we ship** — the one thing in this list that ends up inside an artifact, so record it as vendored-in-output rather than merely used, and note the runtime's own licence text is available at the URL in this plan); `flatpak-cargo-generator.py` from `flatpak/flatpak-builder-tools` at commit `1fc32195e3e60fe5c97f0af646dec7a99df5962b` — the hash named explicitly here, not just "a pinned commit", because this file is the record and the record is the point (MIT by the file's own declaration; used to generate a committed JSON, never copied into the tree); `zsync` (Artistic per its distribution package; build-time only). Also record that `smithay-client-toolkit`'s `xkbcommon` feature was dropped, which removes `xkbcommon` from the dependency tree — the tree's shape is part of what a reader of this file needs.
...
The fourth edit updates the spec's existing §7 row about `rust-version` and `u64::is_multiple_of` — Task 7 measured the number that row was written to wait for, so it is answered rather than left open, in this exact wording:
...
The fifth edit is in §4, not §7: the `wisp doctor` row of the CLI command table gains one appended sentence recording what actually shipped, since Spec 5's author opens the spec, not the plan, and from the spec alone would otherwise build against a doctor that does not exist. It points at this plan's known gaps 8 and 10, in exactly this wording:

```
As implemented, `wisp doctor` also accepts the five value flags `run` forwards (`--log`, `--logs-dir`, `--spells`, `--scale`, `--backend`), prints a seventh `scale:` line, stats the log it names (missing → exit 1), and reports the backend by the HUD's full precedence — flag, then config, then `wisp-probe` detection — naming what detection alone would have chosen; see the plan's known gaps 8 and 10.
```

These five edits do not read as five hunks under a default `git diff`: the two new §7 rows sit directly beside each other, and the fourth edit sits directly beside the existing `Tailer::poll` row it neighbours, so those three merge into one hunk; the fifth edit, in §4's command table, is not adjacent to any of the other four and stands on its own — measured: `git diff docs/specs/` shows 3 hunks. Verify with `git diff -U0 docs/specs/` instead, which shows the real count: five changed regions, one per edit.
...
Each number, path, version and SHA you wrote must trace to this plan's Global Constraints or to a command you ran in an earlier task. `grep` the docs for `0.1.0`, `25.08`, `1.9.1`, `20251108`, `1fc3219` and confirm each is where it should be and nowhere it should not — `1fc3219` must hit **both** `PROVENANCE.md`'s dated entry and `THIRD_PARTY.md`'s generator entry, which each name the commit explicitly rather than saying only "a pinned commit."
...
- [ ] The spec file changed in exactly five places — the Status line, the two new §7 risk rows, the updated `rust-version`/`u64::is_multiple_of` row, and the appended sentence on the `wisp doctor` command-table row: `git diff -U0 docs/specs/` shows exactly five changed regions and nothing else. (A default `git diff` shows fewer: two of the five edits sit adjacent with no context line between them and merge into one hunk. `-U0` is the check that does not miss that.)
...
7. **No `rust-version` is declared, and this is now checked rather than predicted.** `u64::is_multiple_of` stabilised in Rust 1.87; the Flatpak SDK ships `rustc 1.98.1 (48a229cea 2026-09-01)`, and the offline build against it succeeded without a `rust-version` declaration, so none was added.
...
Both tests assert rather than print, so the numbers live in the test bodies and Task 3 must keep them true. Two details of this block are pre-Task-3 facts that legitimately move: `83 filtered out` is **91** as merged (see the test counts below), and the single `Running unittests src/main.rs` block gains a second one for `tests/logs_dir.rs` (Task 3, Step 4). The numbers are, in `crates/wispd/src/encounter.rs`'s `fixture_replay_matches_the_reference_exactly`: `encounters` 2524; `dmg_out` melee/spell/dot/shield 15198942/9082968/4622907/447208; `own_self_dmg` 18085260 and `own_pet_dmg` 5284285; `taken` melee/spell/dot/shield 1924867/709376/273054/257300; `heal_actual` 2364526 and `heal_over` 838539; `own_heal_actual`/`own_heal_over`/`own_hot_actual` 1875603/590911/355478; `heal_outside_fight` 259121; `pet_announcements` 4890 with `pet_count()` 91; `boundaries` 971; `group_member_lines`/`group_leaves`/`group_resets` 40/4/7 and `group_size()` 0 at end of file; durations min/median/max/sum 1/36/738/146813; the largest fight by own damage 212467/482/441/21416; top damage sources `you` 23369545, `Yder` 1447129, `Serenitee` 1321322, `Misery` 1000700; top healers `you` 1875603, `Serenitee` 196052, `Misery` 116859. And in `crates/wispd/src/timers.rs`'s test of the same name: `table.len()` 12245 and the sixteen `TrackerStats` fields 8042, 730, 399, 5972, 822, 214, 4936, 2097, 12814, 1660, 19, 2042, 2107, 144, 1060, 600, with `Mesmerization` VI samples `[22, 27, 21, 27, 19, 28, 24, 22, 28]`, `measured` 24, and `measured` 69/38/57/50 for `Pacify` V, `Venom of the Snake`, `Envenomed Bolt` X, `Odium` X.

**Test counts as merged at `f692794`** (Task 5), counted by `#[test]` attribute in each crate and confirmed against the orchestrator's figures: `wisp` **34 unit + 18 integration**; `wisp-config` **57**; `wispd` **93** — 90 runnable plus the 3 ignored — **and 9 integration** in `tests/logs_dir.rs`; `wisp-probe` 15; `wisp-hud` 36; `wisp-proto` 14. The replay guard therefore reports **91 filtered out** (93 minus the 2 it runs), which supersedes both the 83 measured before Task 3 and the 90 this plan first predicted: the prediction counted eight new unit tests minus the one deleted with `spells_dir_from_log`, and Task 3's fix wave added one more to `wispd` itself. The counts in **Verified facts** above are the pre-implementation ones and stay there as the baseline the Task 2 and Task 4 arithmetic refers to.
...
**Task 7 measurements, from the Flatpak build and checks.** `cargo-sources.json` holds 131 entries: 65 archive sources plus 65 inline `.cargo-checksum.json` entries (one pair per registry crate) plus one `cargo config` source — and, correctly, none of the six workspace crates (`wisp-proto`, `wisp-config`, `wisp-probe`, `wispd`, `wisp-hud`, `wisp`). `Cargo.lock` holds 71 packages: 65 registry crates plus the 6 workspace crates. The SDK's `rustc --version` is `rustc 1.98.1 (48a229cea 2026-09-01)` (the development box has 1.94.1); the offline build succeeded without it, so no `rust-version` was added. `check.sh` output: `PASS version`, `PASS instance-a-starts`, `PASS status-sees-daemon`, `PASS display`, `PASS appstream`, `PASS desktop-file`, with `DISPLAY=:0` and `/tmp/.X11-unix` holding only `X0`. `flatpak ps` may still list a teardown instance for a second or two after `check.sh` returns; the Task 7 self-review's leak check must wait or re-poll rather than fail on the first read. A `wispd.sock` left under `$XDG_RUNTIME_DIR/app/io.github.jds300.Wisp/` by a SIGKILLed daemon is recovered by `Server::bind`'s live-socket check (Task 5) and discarded anyway once the runtime tmpfs clears. `flatpak-builder-tools` at the pinned commit has no root `LICENSE`: fetching one returns HTTP 404, measured directly rather than inferred from a directory listing. The nested `--state-dir` attempt (`$build_dir/.flatpak-builder`) was run and failed with `Refusing to delete current working directory, state directory or their parents`, which is why `build_dir` and `state_dir` are siblings under the repository root instead; an off-root invocation (`cd /tmp && …/packaging/flatpak/build.sh`) was also run with the sibling layout and left nothing under `/tmp` or the repository root once it finished.

**Task 8 runner facts, verified by reviewer A against the `ubuntu-24.04` GitHub-hosted runner image.** Already present: `curl` 8.5.0, `binutils` 2.42 (so `strip` needs no install step), `rustup` 1.29.0, and `zsync` 0.6.2-5build1 — the last is why the `zsyncmake` install in both workflows is guarded rather than unconditional. No `musl-tools` package is needed: this is a pure-Rust workspace and the musl target links self-contained, with nothing to `cc`-link against libc. Neither `desktop-file-utils` nor `appstream` is on the image, so both `desktop-file-validate` and `appstreamcli validate --no-net` take the `SKIP` branch in `ci.yml`, every run, until the image changes. The runner's `toolchain: stable` resolved to `rustc 1.98.1 (48a229cea 2026-09-01)` on 2026-09-10, measured from the first push run's clippy failure; both workflows now pin `toolchain: 1.94.1`, the development box's version, instead.
...
**Task 9 measurements, from the docs run.** `git diff --numstat`: `PROVENANCE.md` +25/−1, `README.md` +39/−1, `THIRD_PARTY.md` +23/−0, `docs/specs/2026-09-09-spec-4-packaging.md` +4/−2 — 91 insertions, 4 deletions across the four documents (independently verified against the commit object). All gates green on the docs-only tree. The Step 2 greps' hits, as reported: `0.1.0` only in pre-existing spec lines; `25.08` in the README's Flatpak installing bullet and the spec; `1.9.1` in `THIRD_PARTY.md` and `PROVENANCE.md`; `20251108` in `THIRD_PARTY.md`; `1fc3219` zero hits, as expected — both `PROVENANCE.md` and `THIRD_PARTY.md` said only "a pinned commit" at the time of this run, which is item 2's finding above and is why `THIRD_PARTY.md`'s generator entry is being changed to name the commit explicitly.

**Whole-branch review measurements, from reviewer A.** 273 tests across the workspace's eight binaries; 30 new files, every one carrying an SPDX header; 21 commits on the branch, every one with the canonical trailer as its last line. `APPIMAGE_EXTRACT_AND_RUN=1 ./dist/Wisp-<v>-x86_64.AppImage --version` prints `wisp 0.1.0`. `pacman -Qi zsync` reports `0.6.6-1.1` and `LicenseRef-Artistic`, matching `THIRD_PARTY.md`. The type-2 runtime's `LICENSE` URL returns HTTP 200 with the MIT text, confirming the licence recorded for the one tool this project embeds in a shipped artifact.
