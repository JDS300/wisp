...
```
  ...
  **A flag given twice keeps the first**, in `run` and in `doctor` alike, because that is what `wispd` and `wisp-hud` do with `iter().position(…)` — and a report that disagrees with the launch it describes is worse than no report. One shared helper resolves a flag's value for both commands, so the rule cannot be stated twice and drift apart.
