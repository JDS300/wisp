...
| 2026-09-11 | T8 | Correction to the T5 row above: it computes the delta against the plan's originally recorded baseline as "+3,328,600 over the plan's originally recorded 3,740,344", but 7,069,344 − 3,740,344 = 3,329,000, not 3,328,600. The row's other number (+3,324,904, against T5's own back-to-back baseline of 3,744,440) is arithmetically correct and unaffected; only the second, parenthetical figure was off. The ledger is append-only, so the T5 row is left as written and this row records the correction | `printf '%d\n' $((7069344 - 3740344))` = `3329000`, checked against the T5 row's own two source numbers |
| 2026-09-11 | T9 | Correction to the T4 row above: it lists `tracing` among the crates `ksni` brought into `Cargo.lock`. `tracing` 0.1.44 was already in `Cargo.lock` at `6c7f1d6`, before Spec 6 touched anything; only `tracing-attributes` is new. `THIRD_PARTY.md` already records this correctly (`tracing-attributes` only) — only the T4 ledger row was wrong | `git show 6c7f1d6:Cargo.lock \| grep -A1 '^name = "tracing"$'` shows `tracing` present at that commit; `git diff 6c7f1d6 -- Cargo.lock \| grep '^+name'` shows `tracing-attributes` as the only `tracing*` addition |
| 2026-09-11 | T9 | Known limitation, not fixed by this task: a `wl_keyboard.leave` event that arrives while HUD mode is active leaves `keyboard_taken` `true` until a later `enter` returns the keyboard, because the layer-shell backend has no independent signal that focus was revoked. The tray's `Activate` (left click, `ToggleHudMode`) is the escape a user has in that state: it drives the render loop's own mode toggle regardless of what the compositor thinks it owns. Milestone 8's interactive pass watches for this | read of `crates/wisp-hud/src/backend/layer_shell.rs`'s keyboard-enter/leave handling; no code changed |
| 2026-09-11 | T9 | F1: with `[profile.release] lto = "fat"` and `codegen-units = 1` added to the root `Cargo.toml`, stripped musl `wisp-hud` at head (`d17c853` plus this task's fixes) = **5,888,416 bytes**, `static-pie linked`, `ldd` reports `statically linked`; `wisp` and `wispd` remain `static-pie linked`. This matches the reviewer's figure exactly. Against the reviewer's LTO baseline (tray unreachable, same profile) of 3,437,104 bytes, the delta is **+2,451,312 bytes**, under spec §5 Milestone 4's "under 3 MB added, stripped" line. `panic = "abort"` was not added, per the controller's ruling. `git status --porcelain` after this build shows no `Cargo.lock` diff: a `[profile.release]` section changes no dependency and no resolved version | `cargo build --release --workspace --locked --target x86_64-unknown-linux-musl && file target/x86_64-unknown-linux-musl/release/{wisp,wispd,wisp-hud} && ldd target/x86_64-unknown-linux-musl/release/wisp-hud && cp target/x86_64-unknown-linux-musl/release/wisp-hud <scratch>/hud-lto && strip <scratch>/hud-lto && stat -c%s <scratch>/hud-lto` = `5888416`; `git status --porcelain` |
...
9. **ksni 0.3.6's licence is Unlicense, not the MIT spec §4.2 asserted.** (plan, T8) — Cost if wrong: a wrong licence line in `THIRD_PARTY.md`; re-derived from `cargo metadata --locked` at review time and confirmed.
10. **Parallel implementers ran in isolated git worktrees, overriding the one-at-a-time execution rule, on JDS300's standing instruction.** (controller) — Cost if wrong: one rebase per collision; every collision was pre-mapped and none needed a hand-merged `Cargo.lock`.
11. **The tray's Stop fix belongs in the daemon, not the tray: `accept_pending` honours a request from a client that closed before the first snapshot write.** (controller, T2 fix round 1) — Cost if wrong: one extra read per accept; without it `echo stop | socat` and the tray's write-and-drop are silently ignored, which spec §4.1 forbids.
12. **The stale `#[allow(dead_code)]` pair and its now-false comment in `backend/mod.rs` were folded into T6b's fix round rather than deferred.** (controller) — Cost if wrong: none.
13. **The musl size delta was re-measured at head against a git archive of the plan commit and is +3,390,440 bytes as shipped, superseding T5's +3,324,904 and T8's +3,329,000.** (T9) — Cost if wrong: the branch ships over spec §5 Milestone 4's line without saying so.
14. **`[profile.release] lto = "fat"` / `codegen-units = 1` was adopted, bringing the delta under the line (measured: shipped `wisp-hud` 5,888,416 bytes stripped, +2,451,312 over the LTO baseline of 3,437,104, still `static-pie linked` and `statically linked`).** (T9, controller ruling) — Cost if wrong: ~8 s of extra release-build time.
15. **A `wl_keyboard.leave` during HUD mode leaves `keyboard_taken` true until an `enter` returns; the tray's Activate is the documented escape.** (T9) — Cost if wrong: a compositor that revokes focus mid-mode leaves the HUD keyless until the tray icon is clicked; Milestone 8 watches for it.
16. **The brief's expected test counts (wisp-hud 86, wispd 102) were estimates; the real counts are 85 and 104 and no test was removed (0 removed `#[test]` lines against 36 added).** (T6b, T9) — Cost if wrong: none.
