...
| **CI is unverified until the first tag.** | Accepted; `actionlint` locally is the stand-in, and the first real run is recorded as pending like every live milestone. The first push run on 2026-09-10 (run 34479004752) built and tested green under Xvfb and failed only on clippy, because floating stable had become 1.98.1 with a new lint; the toolchain is now pinned. The first `v0.1.0` release run on 2026-09-10 (run 34509152679) failed at the test step because `release.yml` lacked the `cargo build --workspace` that ci.yml has, so the `wisp` CLI tests found no `target/debug/wisp-hud`; the step was added and the tag re-pointed. |
