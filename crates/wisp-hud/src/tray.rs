...
mod tests {
    ...
    fn a_snapshot_becomes_a_tray_state_and_a_lingering_fight_is_not_one() {
        let s = tray_state("0.3.0", &snapshot(Some("eqlog_Daggo_freeport.txt"), Some(fight(true, 42))), true, false);
        assert_eq!(s.log_name.as_deref(), Some("eqlog_Daggo_freeport.txt"));
        assert_eq!(s.fight, Some(42));
        assert!(s.hud_mode);

        let lingering = tray_state("0.3.0", &snapshot(Some("x.txt"), Some(fight(false, 42))), false, false);
        ...
        let nothing = tray_state("0.3.0", &snapshot(None, None), false, false);
